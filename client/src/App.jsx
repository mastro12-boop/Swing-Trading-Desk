  ...
  return (
    <div style={SS.container}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        <div style={SS.tabs}>
          {TF_LABELS.map((tf, i) => <button key={tf} onClick={() => setIdx(i)} style={{ ...SS.tab, color: i === idx ? T_REF.accent : T_REF.dim, borderBottom: i === idx ? `2px solid ${T_REF.accent}` : "2px solid transparent" }}>{tf}</button>)}
        </div>
      ...
      <div style={NW.tabs}>
        {NEWS_CATEGORIES.map(cat => (
          <button key={cat.id} onClick={() => setActiveCategory(cat.id)} style={{
            ...NW.tab,
            color: activeCategory === cat.id ? T_REF.textBright : T_REF.dim,
            background: activeCategory === cat.id ? T_REF.cardBg : "transparent",
          }}>{cat.label}</button>
        ))}
      </div>
...
// NW and EC styles are generated by makeWidgetStyles()
...
// EC styles generated by makeWidgetStyles()
...
// Widget styles - generated from current theme
function makeWidgetStyles() {
  const t = T_REF;
  return {
    SS: {
      container: { background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 10, padding: "14px 12px", backdropFilter: t.bubbles ? "blur(8px)" : "none" },
      header: { display: "flex", alignItems: "center", gap: 6, marginBottom: 10 },
      hIcon: { color: t.accent, fontSize: 12 }, hTitle: { fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", color: t.accent },
      ...
      progressTrack: { height: 2, background: t.border, borderRadius: 1, marginBottom: 10, overflow: "hidden" },
      progressFill: { height: 2, background: t.accent, borderRadius: 1, transition: "width 0.08s linear" },
      list: { display: "flex", flexDirection: "column", gap: 4 },
      row: { display: "grid", gridTemplateColumns: "14px 1fr 52px", alignItems: "center", gap: 4, padding: "3px 0" },
      rank: { fontSize: 9, color: t.dimmer, fontWeight: 700, fontFamily: "monospace" },
      sName: { fontSize: 10, color: t.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
      sVal: { fontSize: 10, fontWeight: 700, fontFamily: "'SF Mono',monospace", textAlign: "right" },
      barTrack: { gridColumn: "1/-1", height: 3, background: t.border, borderRadius: 2, overflow: "hidden", marginTop: 1 },
      barFill: { height: 3, borderRadius: 2, transition: "width 0.3s ease" },
      loadingMsg: { fontSize: 11, color: t.dim, padding: "20px 0", textAlign: "center" },
      footnote: { marginTop: 10, fontSize: 9, color: t.dimmer, textAlign: "center" },
    },
    FG: {
      container: { background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 10, padding: "14px 12px", marginTop: 16, backdropFilter: t.bubbles ? "blur(8px)" : "none" },
      header: { display: "flex", alignItems: "center", gap: 6, marginBottom: 2 },
      hIcon: { color: t.orange, fontSize: 12 }, hTitle: { fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", color: t.orange },
      source: { fontSize: 9, color: t.dimmer, marginBottom: 12, paddingLeft: 18 },
      ...
      mainGaugeTrack: { position: "relative", height: 8, background: t.border, borderRadius: 4, overflow: "visible" },
      mainGaugeFill: { height: 8, borderRadius: 4 },
      mainGaugeNeedle: { position: "absolute", top: -4, transform: "translateX(-50%)", width: 2, height: 16, background: t.textBright },
      needleTri: { width: 0, height: 0, borderLeft: "4px solid transparent", borderRight: "4px solid transparent", borderTop: `5px solid ${t.textBright}`, position: "absolute", bottom: -5, left: -3 },
      mainGaugeLabels: { display: "flex", justifyContent: "space-between", fontSize: 8, color: t.dimmer, marginTop: 3 },
      ...
      rowLabel: { fontSize: 9, color: t.dim, fontWeight: 600 },
      gaugeTrack: { position: "relative", height: 4, background: t.border, borderRadius: 2, overflow: "visible" },
      gaugeFill: { height: 4, borderRadius: 2, transition: "width 0.5s ease" },
      gaugeNeedle: { position: "absolute", top: -2, width: 2, height: 8, background: t.textBright, transform: "translateX(-50%)" },
      ...
      footnote: { marginTop: 10, fontSize: 8, color: t.dimmer, textAlign: "center" },
    },
    NW: {
      container: { background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 10, padding: "12px", marginTop: 16, backdropFilter: t.bubbles ? "blur(8px)" : "none" },
      header: { display: "flex", alignItems: "center", gap: 6, marginBottom: 8 },
      hIcon: { color: t.blue, fontSize: 11 },
      hTitle: { fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", color: t.blue },
      ...
      item: { padding: "6px 0", borderBottom: `1px solid ${t.border}`, textDecoration: "none", display: "block", cursor: "pointer" },
      headline: { fontSize: 10, color: t.text, lineHeight: 1.4, transition: "color 0.15s" },
      meta: { fontSize: 8, color: t.dimmer, marginTop: 2 },
      empty: { fontSize: 10, color: t.dim, padding: "12px 0", textAlign: "center" },
    },
    EC: {
      filterRow: { display: "flex", gap: 4, marginBottom: 10, flexWrap: "wrap" },
      filterBtn: { padding: "5px 10px", fontSize: 10, fontWeight: 600, border: `1px solid ${t.border}`, borderRadius: 4, cursor: "pointer", background: "transparent" },
      searchRow: { position: "relative", marginBottom: 14 },
      searchInput: { width: "100%", padding: "8px 32px 8px 12px", fontSize: 12, background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 6, color: t.textBright, outline: "none" },
      clearBtn: { position: "absolute", right: 8, top: "50%", transform: "translateY(-50%)", background: "none", border: "none", color: t.dim, fontSize: 12, cursor: "pointer" },
      table: { background: t.cardBg, border: `1px solid ${t.cardBorder}`, borderRadius: 10, overflow: "hidden", backdropFilter: t.bubbles ? "blur(8px)" : "none" },
      headerRow: { display: "flex", padding: "8px 12px", background: t.bg, borderBottom: `1px solid ${t.border}` },
      headerCell: { fontSize: 9, fontWeight: 700, color: t.dim, textTransform: "uppercase", letterSpacing: "0.08em" },
      dateHeader: { padding: "6px 12px", fontSize: 10, fontWeight: 700, color: t.accent, background: t.accentBg, borderBottom: `1px solid ${t.accentBorder}` },
      row: { display: "flex", padding: "6px 12px", borderBottom: `1px solid ${t.border}`, alignItems: "center" },
      tickerCell: { fontSize: 11, fontWeight: 800, color: t.textBright, fontFamily: "'SF Mono',monospace" },
      nameCell: { fontSize: 10, color: t.muted, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" },
      cell: { fontSize: 10, color: t.muted },
      timeCell: { fontSize: 9, fontWeight: 700, letterSpacing: "0.06em" },
      empty: { padding: "30px 20px", textAlign: "center", background: t.cardBg, borderRadius: 10, border: `1px solid ${t.cardBorder}`, fontSize: 12, color: t.dim },
      count: { padding: "8px 12px", fontSize: 9, color: t.dimmer, textAlign: "right" },
    },
  };
}
